...
import {
  ...
  Copy, CheckCircle, BarChart3, RefreshCw
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [autoRunVendorId, setAutoRunVendorId] = useState(null);
  ...
  // Auto-run pipeline when a new vendor is onboarded
  useEffect(() => {
    if (autoRunVendorId) {
      handleRunPipeline(autoRunVendorId);
      setAutoRunVendorId(null);
    }
  }, [autoRunVendorId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleOnboard = useCallback(async (e) => {
    ...
    try {
      ...
      toast.success('Vendor onboarded! Starting AI pipeline automatically…');
      setAutoRunVendorId(vendorData.vendor_id);
    } catch (err) {
