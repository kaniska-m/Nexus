...
import { X, FileText, CheckCircle, AlertTriangle, ShieldCheck, ShieldAlert, ShieldX, Clock, ExternalLink, Copy, XCircle, Info, Download } from 'lucide-react';
...
export default function VendorDetailDrawer({ isOpen, onClose, vendor }) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        style={{ width: isOpen ? '480px' : '0px', maxWidth: '100vw' }}
        className={`fixed top-0 right-0 h-full bg-slate-50 shadow-2xl z-[70] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden flex flex-col tracking-tight border-slate-200 ${isOpen ? 'border-l' : ''}`}
      >
        ...
        {/* Tabs */}
        <div className="flex border-b border-slate-200 relative shrink-0 bg-white">
          <button onClick={() => setActiveTab('overview')} className={`flex-1 py-3 text-sm font-bold transition-colors ${activeTab === 'overview' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>Overview</button>
          <button onClick={() => setActiveTab('audit')} className={`flex-1 py-3 text-sm font-bold transition-colors ${activeTab === 'audit' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>Audit Trail</button>
          <div className="absolute bottom-0 left-0 w-1/2 h-0.5 bg-blue-600 transition-transform duration-300" style={{ transform: activeTab === 'overview' ? 'translateX(0)' : 'translateX(100%)' }} />
        </div>

        {/* Body Container */}
        <div className="flex-1 overflow-hidden relative w-full bg-slate-50">
          <div className="absolute inset-0 flex w-[200%] transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)]" style={{ transform: activeTab === 'overview' ? 'translateX(0)' : 'translateX(-50%)' }}>
            
            {/* Overview Tab Content */}
            <div className="w-1/2 h-full flex-shrink-0 overflow-y-auto p-5 space-y-5">
              {/* Risk Score Card */}
              <div className="rounded-xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)] border border-slate-200">
            ...
            <div className="w-1/2 h-full flex-shrink-0 flex flex-col pt-5">
              <div className="flex-1 overflow-y-auto px-5 relative pb-5">
                {/* Vertical Timeline Line */}
                {(vendor.audit_log && vendor.audit_log.length > 0) && (
                   <div className="absolute left-[35px] top-[10px] bottom-[10px] w-0.5 bg-slate-200 rounded-full" />
                )}
                
                <div className="space-y-6 relative z-10">
                  {(vendor.audit_log || []).slice().reverse().map((log, i) => (
                    <div key={i} className="flex gap-4 group">
                      <div className="w-8 h-8 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center shrink-0 mt-0.5 shadow-sm group-hover:border-blue-400 group-hover:text-blue-500 transition-colors bg-white">
                        <Clock className="w-3.5 h-3.5 text-slate-400 group-hover:text-blue-500" />
                      </div>
                      <div className="flex-1 bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm leading-snug">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{log.agent}</span>
                          <span className="text-[10px] text-slate-400 font-mono">
                            {new Date(log.created_at || log.timestamp || Date.now()).toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit'})}
                          </span>
                        </div>
                  ...
                  {(!vendor.audit_log || vendor.audit_log.length === 0) && (
                    <div className="text-center py-12">
                      ...
                      <p className="text-sm font-bold text-slate-600 font-syne">No Audit Logs</p>
                      <p className="text-xs text-slate-400 mt-1">Run the pipeline to generate audit history.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-5 border-t border-slate-200 bg-white">
                <button onClick={() => window.open(`/api/vendor/${vendor.vendor_id}/audit-pdf`, '_blank')} className="w-full nexus-btn-outline py-2.5 flex items-center justify-center gap-2 text-blue-600 border-blue-200 hover:bg-blue-50 hover:border-blue-300 shadow-sm">
                  <Download className="w-4 h-4" /> Download Certified Audit PDF
                </button>
              </div>
            </div>
